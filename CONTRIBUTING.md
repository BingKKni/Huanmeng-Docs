...
感谢您愿意花时间改进幻梦机器人的相关文档。下面说明如何从本地修改到提交 Pull Request（PR）。
...
| `src/hmdoc/` | 机器人帮助文档（如 `index.md` 等） |
...
浏览器访问终端里提示的地址（本仓库脚本默认为 **`http://localhost:23333`**，且监听 `0.0.0.0`，便于局域网其它设备访问）。
...
1. 新建一个分支（示例：`fix/typo-hmdoc` 或 `docs/update-invite`），**不要直接在默认分支上堆大量无关修改**。
